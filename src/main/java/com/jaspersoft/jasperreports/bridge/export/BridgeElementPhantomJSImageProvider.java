...
import static com.jaspersoft.jasperreports.bridge.export.BridgeElementImageProvider.copyStream;
 ...
 */
public class BridgeElementPhantomJSImageProvider extends BridgeElementImageProvider
{
    
        private static final Log log = LogFactory.getLog(BridgeElementPhantomJSImageProvider.class);
        private static final Random RANDOM = new Random();
    
        public static final String PROPERTY_PHANTOMJS_EXECUTABLE_PATH = "com.jaspersoft.jasperreports.components.bridge.phantomjs.executable.path";
	...
	public static final String PROPERTY_PHANTOMJS_TEMPDIR_PATH = "com.jaspersoft.jasperreports.highcharts.phantomjs.tempdir.path";
        ...
        public static final String DIV2SVG_SCRIPT = "com/jaspersoft/jasperreports/bridge/scripts/div2svg.js";
        public static final String SVG2PNG_SCRIPT = "com/jaspersoft/jasperreports/bridge/scripts/svg2png.js";
        public static final String ERROR_SVG = "com/jaspersoft/jasperreports/bridge/export/error.svg";
	...
	public byte[] createPngImage(JasperReportsContext jasperReportsContext, JRGenericPrintElement element) {
        ...
        public String createSvgImage(JasperReportsContext jasperReportsContext, JRGenericPrintElement element) {
                try {            
                    ...
                    log.error("Generating image for Bridge element " + element.hashCode() +" failed.", ex);
        ...
        public static String createErrorSVG(String error)
        {
            try {
                ...
                copyStream( BridgeElementPhantomJSImageProvider.class.getClassLoader().getResourceAsStream(ERROR_SVG) , os);
        ...
        public byte[] getImageData(JasperReportsContext jasperReportsContext, JRGenericPrintElement element, boolean svg) throws Exception {
                ...
                if (element.getParameterValue( BridgePrintElement.CONFIGURATION) == null) throw new JRRuntimeException("Configuration object is null.");
            
		String phantomjsExecutablePath = "phantomjs"; 
                
                if (jasperReportsContext.getProperty(BridgeElementPhantomJSImageProvider.PROPERTY_PHANTOMJS_EXECUTABLE_PATH) != null)
                {
                    phantomjsExecutablePath = jasperReportsContext.getProperty(BridgeElementPhantomJSImageProvider.PROPERTY_PHANTOMJS_EXECUTABLE_PATH);
                }
		
                int phantomjsTimeout; 
                String timeoutProperty = jasperReportsContext.getProperty(BridgeElementPhantomJSImageProvider.PROPERTY_PHANTOMJS_EXECUTABLE_TIMEOUT);
		...
		String phantomjsTempFolderPath = jasperReportsContext.getProperty(BridgeElementPhantomJSImageProvider.PROPERTY_PHANTOMJS_TEMPDIR_PATH);
		...
		if (phantomjsTempFolderPath == null) {
			...
			String tempFileName = TEMP_RESOURCE_PREFIX + System.currentTimeMillis() + "_" + RANDOM.nextInt(Integer.MAX_VALUE);
                        ...
                        try {
                                ...
                                log.debug("Saving Div2Svg.js in a temporary directory: " + BridgeElementPhantomJSImageProvider.class.getClassLoader().getResource(DIV2SVG_SCRIPT) + " " + new File(tempFolder, tempFileName+ "_div2svg.js"));
                                File div2svgFile = createFile(new File(tempFolder, tempFileName+ "_div2svg.js"), BridgeElementPhantomJSImageProvider.class.getClassLoader().getResourceAsStream(DIV2SVG_SCRIPT));
                                cleanableResourcePaths.add(div2svgFile);

                                File svg2pngFile = createFile(new File(tempFolder, tempFileName+ "_svg2png.js"), BridgeElementPhantomJSImageProvider.class.getClassLoader().getResourceAsStream(SVG2PNG_SCRIPT));
                                ...
                                String htmlFragment = BridgeElementHtmlHandler.getInstance().getHtmlFragment(jasperReportsContext, null, element);
                            ...
                            for (File cleanableResource: cleanableResourcePaths) {
                                if (cleanableResource.exists() && cleanableResource.canWrite()) {
                                        if (element.getPropertiesMap().getProperty("bridge.keepTemporaryFiles") == null ||
