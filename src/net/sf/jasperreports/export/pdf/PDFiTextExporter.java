 ...
 */
public class PDFiTextExporter extends JRAbstractExporter
{

	private static final Log log = LogFactory.getLog(PDFiTextExporter.class);
	...
	protected class ExporterContext extends BaseExporterContext implements PDFiTextExporterContext
	{
	...
	protected PDFiTextExporterTagHelper tagHelper = new PDFiTextExporterTagHelper(this);
	...
	protected PDFiTextExporterContext exporterContext = new ExporterContext();
	...
	protected void setSplitCharacter()
	{
		...
		{
			splitCharacter = new PDFiTextBreakIteratorSplitCharacter();
		}
	}
			...
			if (renderer.getType() == JRRenderable.TYPE_IMAGE)
			{
				...
				switch(printImage.getScaleImageValue())
				{
					case CLIP :
					{
						...
						//image = com.itextpdf.text.Image.getInstance(awtImage, printImage.getBackcolor());
						image = com.itextpdf.text.Image.getInstance(bi, null);
					...
					case FILL_FRAME :
					{
						...
						{
							image = (com.itextpdf.text.Image)loadedImagesMap.get(renderer);
						}
						else
						{
							try
							{
								image = com.itextpdf.text.Image.getInstance(renderer.getImageData());
	 ...
	 *
	 */
	protected Paragraph getParagraph(JRPrintText textElement, AttributedCharacterIterator iterator, String text)
	{
	 ...
	 *
	 */
	protected Phrase getPhrase(JRPrintText textElement, AttributedCharacterIterator iterator, String text)
	{
	...
	protected Chunk getChunk(Map attributes, String text, Locale locale)
	{
		...
		if (backcolor != null)
		{
			chunk.setBackground(new BaseColor(backcolor));
		}
...
//				}
		...
		try
		{
			font = FontFactory.getFont(
				...
				pdfFontStyle,
				forecolor == null ? BaseColor.BLACK : new BaseColor(forecolor)//FIXMENOW check if this can be optimized; also check the null test everywhere
				);

			// check if FontFactory didn't find the font
			if (font.getBaseFont() == null && font.getFamily() == com.itextpdf.text.Font.FontFamily.UNDEFINED)//FIXMENOW check this equal
			{
				font = null;
			}
		}
	...
	public void exportText(JRPrintText text) throws DocumentException
	{
		...
		if (textLength > 0)
		{
			...
			StringTokenizer tkzer = new StringTokenizer(styledText.getText(), "\n", true);
			
			while(tkzer.hasMoreTokens())
			{
				String paragraphText = tkzer.nextToken();
				
				parEndIndex = parStartIndex + paragraphText.length();
				...
				AttributedCharacterIterator paragraph = new AttributedString(allParagraphs, parStartIndex, parEndIndex).getIterator();
				
				int currentTab = 0;
				
				float horizontalPos = 0;
				float lcVerticalPos = verticalPos;
				int startIndex = 0;
				int tabIndexOrEndIndex = 0;
				
				while (tabIndexOrEndIndex < paragraphText.length())
				{
					tabIndexOrEndIndex = (tabIndexes == null || currentTab >= tabIndexes.size() ? paragraphText.length() : (tabIndexes.get(currentTab) + 0));

					AttributedCharacterIterator chunkPar = new AttributedString(paragraph, startIndex, tabIndexOrEndIndex).getIterator();
					
					if (startIndex < tabIndexOrEndIndex)
					{
						//Phrase phrase = getParagraph(text, paragraph, styledText.getText().substring(startIndex, tabIndexOrEndIndex));
						Paragraph para = getParagraph(text, chunkPar, paragraphText.substring(startIndex, tabIndexOrEndIndex));
						
						para.setFirstLineIndent(horizontalPos);// - x - leftPadding);
						para.setLeading(0, text.getLineSpacingFactor());
						
						ColumnText2 colText = new ColumnText2(pdfContentByte);
						colText.addElement(para);
						colText.setSimpleColumn(
							//para,
							x + leftPadding,
							lcVerticalPos,
						...
						//colText.setUseAscender(true);
						//colText.setExtraParagraphSpace(-100);
						colText.setAlignment(horizontalAlignment);
						...
						{
							float colHeight = lcVerticalPos - colText.getYLine();
							float lineHeight = colHeight / colText.getLinesWritten();
							
							lcVerticalPos = colText.getYLine() + lineHeight;// + colText.getCurrentLeading();// - colText.getExtraParagraphSpace();
							
							//lcVerticalPos = colText.getYLine() + colText.getCurrentLeading();// - colText.getExtraParagraphSpace();
						}
						
						verticalPos = colText.getYLine();
					}
					
					if (tabIndexOrEndIndex < paragraphText.length())
					{
						// the chunk limit was a tab; going to the next tab
						currentTab++;
						
						//colText.addText(new Chunk(new DottedLineSeparator(), ((int)(colText.getLastX() / tabStopWidth) + 1) * tabStopWidth));
						//para.add(new Chunk(new DottedLineSeparator(), ((int)(colText.getLastX() / tabStopWidth) + 1) * tabStopWidth));
					}
	...
	{
		PDFiTextGenericElementHandler handler = (PDFiTextGenericElementHandler) 
...
class ColumnText2 extends ColumnText
{
	public ColumnText2(PdfContentByte pdfContentByte)
	{
