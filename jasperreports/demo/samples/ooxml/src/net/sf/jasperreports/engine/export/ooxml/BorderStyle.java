...
public abstract class BorderStyle extends Style
{
	 ...
	 *
	 */
	public BorderStyle(Writer bodyWriter, JRPrintElement element)
	{
	...
	public void setBox(JRLineBox box) throws IOException
	{
		...
		borderPadding[TOP_BORDER] = String.valueOf(Utility.translatePointsToTwips(box.getTopPadding().intValue()));
		appendBorder(box.getLeftPen(), LEFT_BORDER);
		borderPadding[LEFT_BORDER] = String.valueOf(Utility.translatePointsToTwips(box.getLeftPadding().intValue()));
		appendBorder(box.getBottomPen(), BOTTOM_BORDER);
		borderPadding[BOTTOM_BORDER] = String.valueOf(Utility.translatePointsToTwips(box.getBottomPadding().intValue()));
		appendBorder(box.getRightPen(), RIGHT_BORDER);
		borderPadding[RIGHT_BORDER] = String.valueOf(Utility.translatePointsToTwips(box.getRightPadding().intValue()));
	}
	...
	{
		return	borderWidth[TOP_BORDER] != null
			|| borderWidth[LEFT_BORDER] != null
			|| borderWidth[BOTTOM_BORDER] != null
			|| borderWidth[RIGHT_BORDER] != null;
	}
	...
	protected void writeBorder(int side) throws IOException
	{
		...
		{
			bodyWriter.write("       <w:" + border[side] +" w:val=\"" + borderStyle[side] + "\" w:sz=\"" + borderWidth[side] + "\" w:space=\"0\"	w:color=\"" + borderColor[side] + "\" /> \r\n");
			bodyWriter.flush();
		}
	}
	/**
	...
	protected void writePadding(int side) throws IOException
	{
		...
		{
			bodyWriter.write("       <w:" + border[side] +" w:w=\"" + borderPadding[side] + "\" w:type=\"dxa\" /> \r\n");
			bodyWriter.flush();
		}
	}
