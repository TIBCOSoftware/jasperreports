define(["jive", "jasperreports"], function(jive, jasperreports) {
	jive.interactive.column = {
	    ...
	    init: function(allColumns, tableUuid){
	        ...
	        for (i = 0; i < tableCols.length; i++) {
	        	var colUuid = jQuery(tableCols.get(i)).data('coluuid'),
	        ...
	        jasperreports.events.subscribeToEvent({
	        		name: 'jive.ui.foobar', 
	        		/*callback: 'jive.ui.foobar.reset', 
	        		thisContext: 'jive.ui.foobar'
	        		*/	
	        		callback: function() {
	        			require(["jive"], function(jive) {
	    ...
	    getElementSize: function(){
	        ...
	        jo.parents().each(function(i, v) {
	        	var lastCell = jQuery('td.cel_' + cid + ':last', v);
	    ...
	    hide: function(args){
	    	var it = this, tableUuid = jive.selected.jo.data('tableuuid');
	        jive.hide();
	        jive.runAction({
	        	actionData: {
