		...
		showMap: function(canvasId, latitude, longitude, zoom, mapType, markers) {
			...
			if(markers){
				...
				for (var i = 0; i < markers.length; i++) {
				    ...
				    if(markerProps['icon.url'] && markerProps['icon.url'].length > 0) configureImage('icon', markerProps, markerOptions);
				    ...
				    if(markerProps['shadow.url'] && markerProps['shadow.url'].length > 0) configureImage('shadow', markerProps, markerOptions);
				    ...
				    marker['info'] = createInfo(markerProps);
...
function configureImage (pk, pp, po) {
