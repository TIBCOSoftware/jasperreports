	...
	private static final ExtensionsRegistry extensionsRegistry = 
		new ExtensionsRegistry()
		{
			...
			public <T> List<T> getExtensions(Class<T> extensionType) 
			{
				...
				{
					return (List<T>) Collections.singletonList((Object)HibernateQueryExecuterFactoryBundle.getInstance());
				}
				else if (DataAdapterContributorFactory.class.equals(extensionType))
				{
					return (List<T>) Collections.singletonList((Object)HibernateDataAdapterServiceFactory.getInstance());
				}
				return null;
			}
		};
