	...
	private static final ExtensionsRegistry extensionsRegistry = 
		new ExtensionsRegistry()
		{
			...
			public <T> List<T> getExtensions(Class<T> extensionType) 
			{
				...
				{
					return (List<T>) Collections.singletonList((Object)OlapQueryExecuterFactoryBundle.getInstance());
				}
				if (DataAdapterContributorFactory.class.equals(extensionType))
				{
					return (List<T>) Collections.singletonList((Object)MondrianDataAdapterServiceFactory.getInstance());
				}
				return null;
			}
		};
